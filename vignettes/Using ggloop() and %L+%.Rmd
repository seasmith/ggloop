--- 
title   : "Using `ggloop()` and `%L+%`" 
author  : "Luke Smith" 
date    : "`r Sys.Date()`" 
output  : rmarkdown::html_vignette 
vignette: > 
  %\VignetteIndexEntry{Using `gglop()` and `%L+%`} 
...
Let's see what `ggloop()` can (and cannot) do using using baseball statistics
from the Lahman package. We will looking at data for the Houston Astros.

```{r "Load and subset", cache=FALSE}
library(magrittr)
library(Lahman)
HOU <- Teams[Teams$franchID == "HOU", ]
```
...
- Never uses retina figures - Has a smaller default figure size - Uses a custom
CSS stylesheet instead of the default Twitter Bootstrap style
